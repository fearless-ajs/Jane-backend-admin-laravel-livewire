<div class="card-datatable table-responsive pt-0">
    <div class="card-body border-bottom">
        <h4 wire:loading.remove wire:target="search" class="card-title">@if($searchResult)  {{count($searchResult)}}  @else {{count($contacts)}} @endif Contacts</h4>
        ...
        @if($contacts)
            @foreach($contacts as $contact)
                <tr>
                    ...
                    @if($contact->company)
                        <td> <a href="{{route('admin.company-profile', $contact->company->id)}}">{{$contact->company->name}}</a></td>
                    @else
                        <td>Company not available</td>
                    @endif

                    <td>{{$contact->created_at->diffForHumans()}}</td>
                    <td><a href="{{route('admin.contact-profile', $contact->id)}}">Details</a> </td>
