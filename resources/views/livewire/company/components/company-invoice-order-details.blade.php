...
{{--                                <p class="card-text">Ordered By: <span> {{$item->invoiceOrder->user->firstname}} {{$item->invoiceOrder->user->lastname}}</span></p>--}}
{{--                               --}}
                        ...
                        @if($item->catalogue->type === 'product')
                            <div class="d-flex flex-column flex-sm-row pt-1">
                                @if($item->pipeline != 'delivered')
                                    @if($item->pipeline == 'cancelled')
                                        <button type="button" wire:loading.remove wire:target="resume({{$item->id}})" class="btn btn-primary me-0 me-sm-1 mb-1 mb-sm-0" wire:click="resume({{$item->id}})">
                                            Resume order
                                        </button>
{{--                                        <button type="button" wire:loading wire:target="resume({{$item->id}})" disabled class="btn btn-outline-warning me-0 me-sm-1 mb-1 mb-sm-0">--}}
{{--                                            <span class= "spinner-border spinner-border-sm" role="status" aria-hidden="true"></span> Processing...--}}
{{--                                        </button>--}}
                                    @else
                                        <button type="button" wire:loading.remove wire:target="cancel({{$item->id}})" class="btn btn-danger me-0 me-sm-1 mb-1 mb-sm-0" wire:click="cancel({{$item->id}})">
                                            Cancel order
                                        </button>
{{--                                        <button type="button" wire:loading wire:target="cancel({{$item->id}})" disabled class="btn btn-outline-warning me-0 me-sm-1 mb-1 mb-sm-0">--}}
{{--                                            <span class= "spinner-border spinner-border-sm" role="status" aria-hidden="true"></span> Processing...--}}
{{--                                        </button>--}}
                                    @endif
                                @endif
                        ...
                        @if($item->catalogue->type === 'service')
                            @if($item->pipeline == 'cancelled')
                                <button wire:loading.remove wire:target="resumeService" type="button" class="btn btn-primary me-0 me-sm-1 mb-1 mb-sm-0" wire:click="resumeService">
                                    Resume service
                                </button>
{{--                                <button type="button" wire:loading wire:target="resumeService" disabled class="btn btn-outline-warning me-0 me-sm-1 mb-1 mb-sm-0">--}}
{{--                                    <span class= "spinner-border spinner-border-sm" role="status" aria-hidden="true"></span> Processing...--}}
{{--                                </button>--}}
                            @else
                                <button  wire:loading.remove wire:target="cancelService"  type="button" class="btn btn-danger me-0 me-sm-1 mb-1 mb-sm-0" wire:click="cancelService">
                                    Cancel service
                                </button>
{{--                                <button type="button" wire:loading wire:target="cancelService" disabled class="btn btn-outline-warning me-0 me-sm-1 mb-1 mb-sm-0">--}}
{{--                                    <span class= "spinner-border spinner-border-sm" role="status" aria-hidden="true"></span> Processing...--}}
{{--                                </button>--}}
                            @endif
                        @endif
