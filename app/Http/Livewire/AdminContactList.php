...
class AdminContactList extends LiveNotify
{
    use WithPagination;
    ...
    public function updated(){
        if ($this->search){
            $this->searchResult = Contact::where('lastname', 'LIKE', "%{$this->search}%")->orWhere('firstname', 'LIKE', "%{$this->search}%")->get();
        }
    }
    ...
    public function render()
    {
        if ($this->searchResult && !empty($this->search)){
            return view('livewire.admin.components.admin-contact-list', [
